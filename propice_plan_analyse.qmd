...
Il s'agit d'une étude purement observationnelle sans test sur le critère principal. Les pourcentages mesurés seront nombreux, on se base donc sur le taux le plus défavorable soit `r px*100`%. En utilisant un calcul de taille d'échantillon souhaitable pour un sondage en prenant en compte les tests multiples qui vont être réalisés, avec une marge d'erreur de `r ex * 100` % on obtient un nombre minimal d'environ **`r nn` cas** exploitables. 
